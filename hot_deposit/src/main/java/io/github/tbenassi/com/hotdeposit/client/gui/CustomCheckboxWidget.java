...
public class CustomCheckboxWidget extends PressableWidget {
    ...
    public void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (this.showMessage) {
            var textX = getX() - 50;
            var textY = getY() + ((double) (this.height - 8) / 2) + .3;
            float scale = .85f;
            float adjustedTextX = textX + (scale);
            context.getMatrices().push();
            context.getMatrices().translate(adjustedTextX, textY, 0);
            context.getMatrices().scale(scale, scale, 1f);

            context.drawText(textRenderer, this.getMessage(), 0,0, TEXT_COLOR, false);
            context.getMatrices().pop();
        }
    }
